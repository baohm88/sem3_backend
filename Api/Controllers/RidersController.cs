...
public class RidersController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<RiderProfile>>> GetRiderByUserId([FromRoute] string userId)
    {
        ...
        if (p == null) return ApiResponse<RiderProfile>.Fail("NOT_FOUND", "Rider không tồn tại");
        return ApiResponse<RiderProfile>.Ok(p);
    }
    ...
    public async Task<ActionResult<ApiResponse<Order>>> CreateOrder([FromRoute] string userId, [FromBody] CreateOrderDto dto)
    {
        ...
        if (string.IsNullOrWhiteSpace(dto.CompanyId)) return ApiResponse<Order>.Fail("VALIDATION", "CompanyId bắt buộc");
        if (string.IsNullOrWhiteSpace(dto.ServiceId)) return ApiResponse<Order>.Fail("VALIDATION", "ServiceId bắt buộc");

        var company = await _db.Companies.FirstOrDefaultAsync(c => c.Id == dto.CompanyId);
        if (company == null || !company.IsActive) return ApiResponse<Order>.Fail("NOT_FOUND", "Company không hợp lệ");
        var svc = await _db.Services.FirstOrDefaultAsync(s => s.Id == dto.ServiceId && s.CompanyId == dto.CompanyId && s.IsActive);
        if (svc == null) return ApiResponse<Order>.Fail("NOT_FOUND", "Service không hợp lệ");
    ...
    public async Task<ActionResult<ApiResponse<Order>>> CancelOrder([FromRoute] string userId, [FromRoute] string orderId)
    {
        ...
        if (o == null) return ApiResponse<Order>.Fail("NOT_FOUND", "Order không tồn tại");
        if (o.Status == OrderStatus.Completed) return ApiResponse<Order>.Fail("INVALID_STATE", "Order đã hoàn tất");
        if (o.Status == OrderStatus.Cancelled) return ApiResponse<Order>.Fail("INVALID_STATE", "Order đã hủy");
    ...
    public async Task<ActionResult<ApiResponse<Review>>> ReviewOrder([FromRoute] string userId, [FromRoute] string orderId, [FromBody] ReviewOrderDto dto)
    {
        ...
        var o = await _db.Orders.FirstOrDefaultAsync(x => x.Id == orderId && x.RiderUserId == userId);
        if (o == null) return ApiResponse<Review>.Fail("NOT_FOUND", "Order không tồn tại");
        if (o.Status != OrderStatus.Completed) return ApiResponse<Review>.Fail("INVALID_STATE", "Chỉ đánh giá khi Order đã hoàn tất");

        var hasReview = await _db.Reviews.AnyAsync(r => r.OrderId == orderId && r.RiderUserId == userId);
        if (hasReview) return ApiResponse<Review>.Fail("DUPLICATE", "Bạn đã đánh giá order này");
        ...
        // update company rating (simple avg recalculation)
